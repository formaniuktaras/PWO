...
class TemplateEngine:
    ...
    def generate(self, doc_type: DocType, event_id: int, context: dict[str, str]) -> Path:
        ...
        raw_doc_date = context.get("DOC_DATE")
        raw_reg_date = context.get("REG_DATE")
        doc_date = date.fromisoformat(raw_doc_date) if raw_doc_date else event.event_date
        reg_date = date.fromisoformat(raw_reg_date) if raw_reg_date else date.today()
